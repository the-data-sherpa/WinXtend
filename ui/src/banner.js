...
export function bannerFor({ connected, busy, daemon, fault, eventsProblem }) {
  ...
  if (connected) {
    return {
      ...
      hint: "Pairing needs this: a request from another machine will not appear as it is made — it shows up only when this window next reads the agent's status, such as when you bring it to the front or while the Status tab is open — and a pairing started from this window will sit on its code without ever saying whether it was accepted.",
      actions: [],
    };
  }
